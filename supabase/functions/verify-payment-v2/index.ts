...
async function creditWallet(userId: string, amount: number, razorpayOrderId: string) {
  ...
  await fetch(`${supabaseUrl}/rest/v1/wallets?user_id=eq.${userId}`, {
    ...
    body: JSON.stringify({ balance: newBalance, total_recharged: Number((Number(wallet.total_recharged || 0) + amount).toFixed(2)) }),
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!razorpayOrderId || !razorpayPaymentId || !razorpaySignature) {
      return new Response(JSON.stringify({ error: 'All payment verification fields are required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }

    const generatedSignature = await createHmacSha256(razorpayKeySecret, razorpayOrderId + '|' + razorpayPaymentId);
    ...
    const paymentRes = await fetch(`${supabaseUrl}/rest/v1/payments?gateway_payment_id=eq.${razorpayOrderId}&select=*`, { headers });
    const payments = await paymentRes.json();
    const payment = payments?.[0];

    if (!payment) {
      return new Response(JSON.stringify({ error: 'Payment record not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    }
