// WARNING: Deno Edge Functions do NOT support Node's `import('crypto')`.
// Use Web Crypto API via `_shared/hmac.ts` instead (see razorpay-webhook-v2 for reference).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // FIX: Use Web Crypto API instead of Node's import('crypto') which fails in Deno
